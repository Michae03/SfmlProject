    ...
    {
        //ARIAL
        // Utwórz strumień pliku i wczytaj zawartość pliku czcionki Arial
        std::ifstream fontFile("C:\\Users\\Michal\\OneDrive\\Pulpit\\VS\\SFML\\SfmlProject\\SfmlProject\\arial.ttf", std::ios::binary);
        if (!fontFile)
        {
            std::cerr << "Could not open font arial file\n";
        }
        fontFile.seekg(0, std::ios::end);
        const auto fileSize = fontFile.tellg();
        fontFile.seekg(0, std::ios::beg);
        std::vector<char> fontData(fileSize);
        fontFile.read(fontData.data(), fileSize);

        // Utwórz strumień z danymi czcionki

        sf::MemoryInputStream fontStream;
        fontStream.open(fontData.data(), fileSize);

        // Utwórz obiekt czcionki i wczytaj dane czcionki ze strumienia
        if (!arial.loadFromStream(fontStream))
        {
            std::cerr << "Could not load font arial from stream\n";
        }
    }
    ...
    void GameEngine::initText()
    {
        ...
        std::cout << goldText.getFont();
        goldText.setCharacterSize(13);
        goldText.setString("Hello");
        goldText.setPosition(sf::Vector2f(500, 500));
